/**
 * Lightweight QR Code SVG/Canvas Generator for LG webOS TV
 * Based on QRCode generator algorithm (TypeNumber auto-detect, ErrorCorrection Level M)
 */
(function(window) {
  'use strict';

  // Minimal standalone QR Code matrix encoder
  ...
  QRCodeModel.prototype = {
    ...
    make: function() {
      // Automatic type number resolution
  ...
  // Minimal Reed-Solomon tables and polynomials
  ...
  /**
   * Main Public API: render QR code directly into an SVG string or container
   */
  window.QRCodeGenerator = {
    renderSVG: function(text, size) {
      size = size || 250;
      var qr = new QRCodeModel(0, 0); // Type auto-detect, Level M
      qr.addData(text);
      qr.make();
      ...
      for (var r = 0; r < count; r++) {
        for (var c = 0; c < count; c++) {
          if (qr.isDark(r, c)) {
            var x = c * cellSize;
            var y = r * cellSize;
            svg += '<rect x="' + x + '" y="' + y + '" width="' + (cellSize + 0.5) + '" height="' + (cellSize + 0.5) + '" fill="#000000" />';
          }
        }
      }
      svg += '</svg>';
