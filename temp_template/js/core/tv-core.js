...
window.TVCore = {
    ...
    injectHotelLogo: function (config) {
        // Skip logo injection only on pure modal/centered popup pages
        if (document.body.classList.contains('page-centered') || document.querySelector('.config-card') || document.querySelector('.lang-modal-container')) {
            return;
        }
    ...
    /**
     * In-Memory Realtime UI Update Hooks for Flutter host app
     */
    renderMenus: function (menuData) {
        if (typeof window.renderMenus === 'function') {
            return window.renderMenus(menuData);
        }
        if (window.TVHome && typeof window.TVHome.renderMenuTrack === 'function') {
            return window.TVHome.renderMenuTrack(menuData);
        }
        return this.fetchHotelConfig(true).then(config => {
            if (config) {
                this.injectHotelLogo(config);
                this.initBackgroundSlider(config);
            }
